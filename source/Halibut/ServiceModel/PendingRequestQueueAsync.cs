...
namespace Halibut.ServiceModel
{
    public class PendingRequestQueueAsync : IPendingRequestQueue
    {
        ...
        public async Task<ResponseMessage> QueueAndWaitAsync(RequestMessage request, CancellationToken requestCancellationToken)
        {
            ...
            using (await queueLock.LockAsync(requestCancellationToken))
            {
                queue.Add(pending);
            ...
            try
            {
                await pending.WaitUntilComplete(requestCancellationToken);
            }
            finally
            {
        ...
        public async Task<RequestMessageWithCancellationToken> DequeueAsync(CancellationToken cancellationToken)
        {
            ...
            while (true)
            {
                ...
                if (result)
                {
                    return new (pending.Request, pending.InProgressPendingRequestCancellationToken);
                }
            }
        }
        ...
        class PendingRequest : IDisposable
        {
            ...
            bool transferBegun;
            bool completed;
            readonly CancellationTokenSource inProgressPendingRequestCancellationTokenSource;
            ...
            public PendingRequest(RequestMessage request, ILog log)
            {
                ...
                inProgressPendingRequestCancellationTokenSource = new CancellationTokenSource();
                InProgressPendingRequestCancellationToken = inProgressPendingRequestCancellationTokenSource.Token;
            }

            public RequestMessage Request => request;

            public async Task WaitUntilComplete(CancellationToken requestCancellationToken)
            {
                ...
                catch (Exception ex) when (ex is TaskCanceledException or OperationCanceledException)
                {
                    ...
                    // It is possible that the transfer has already started once the requestCancellationTokens.LinkedCancellationToken is cancelled
            ...
            async Task<bool> WaitForResponseToBeSet(TimeSpan timeout, CancellationToken cancellationToken)
            {
                ...
                {
                    // Cancel the Pending Request to force cancellation to the socket if it is currently being transferred 
            ...
            public CancellationToken InProgressPendingRequestCancellationToken { get; }
            ...
            void CancelInProgressRequest()
            {
                inProgressPendingRequestCancellationTokenSource.Cancel();
            }

            public void Dispose()
            {
                inProgressPendingRequestCancellationTokenSource?.Dispose();
